...
module TypeScript {
    ...
    export class PullTypeResolver {
                    ...
                    if (name || (member.kind === PullElementKind.Property && name === "")) {
                        ...
                        if (parentSymbol !== allMembers[i].getContainer()) {
                            ...
                            if (indexerKindMatchesMemberNameKind || onlyStringIndexerIsPresent) {
                                var comparisonInfo = new TypeComparisonInfo();
                                if (!this.sourceIsSubtypeOfTarget(allMembers[i].type, signature.returnType, context, comparisonInfo, /*isComparingInstantiatedSignatures*/ false)) {
        ...
        private inferArgumentTypesForSignature(signature: PullSignatureSymbol, args: ISeparatedSyntaxList, comparisonInfo: TypeComparisonInfo, context: PullTypeResolutionContext): PullTypeSymbol[] {
